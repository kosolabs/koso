...
export function newProjectContext(
  ...
  window.koso = koso;
  window.Y = Y;
  return setProjectContext(
    new ProjectContext(id, koso, socket, visibilityFilterFn, flattenFn),
  );
}

export function newProjectContextNoStore(
  visibilityFilterFn: VisibilityFilterFn,
  flattenFn: FlattenFn,
): ProjectContext {
  const id = page.params.projectId;
  ...
  window.koso = koso;
  window.Y = Y;
  return new ProjectContext(id, koso, socket, visibilityFilterFn, flattenFn);
}
