use super::projects_state::ProjectVersion;
use yrs::{
...
pub(crate) fn sync_request(sv: &StateVector, version: ProjectVersion) -> Vec<u8> {
    let mut encoder = EncoderV1::new();
    encoder.write_var(MSG_SYNC);
    encoder.write_var(MSG_SYNC_REQUEST);
    encoder.write_buf(sv.encode_v1());
    encoder.write_var(version);
    encoder.to_vec()
}

pub(crate) fn sync_response(update: &[u8], version: ProjectVersion) -> Vec<u8> {
    let mut encoder = EncoderV1::new();
    encoder.write_var(MSG_SYNC);
...
pub(crate) fn sync_update(update: &[u8], version: ProjectVersion) -> Vec<u8> {
    let mut encoder = EncoderV1::new();
    encoder.write_var(MSG_SYNC);
