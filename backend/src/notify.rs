...
impl Notifier {
    ...
    fn commit_doc_event(
        ...
        // TODO: Figure out how to ensure ordering of updates.
        ...
        match client {
            Some(mut client) => {
                if let Err(e) = client.close().await {
                    tracing::warn!("Failed to close client: {e}");
                }
            }
            None => {
                tracing::error!(
    ...
    async fn commit_task_update(
        ...
        match update {
            ...
            TaskUpdate::Update { task } => {
                ...
                {
                    Err(e) => {
                        return Err(format!("Failed to update task: {e}, {update:?}").into());
                    }
                    Ok(res) => {
                        if res.rows_affected() == 0 {
    ...
    async fn broadcast(&mut self, update: &YrsUpdate) {
        ...
        for client in clients.values_mut() {
            if client.who != update.who {
                tracing::trace!("Sending update to {}", client.who);
                results.push(client.send(update.data.clone()));
            }
        }
