...
impl Notifier {
    ...
    async fn process_update(&self, update: YrsUpdate) {
        ...
        self.tracker
            .spawn(self.clone().store_doc(update.project_id.clone()));
        self.tracker.spawn(self.clone().broadcast_update(update));
    }
    ...
    async fn store_doc(self, project_id: String) {
        let sv = {
            let Some(project) = self.state.get(&project_id) else {
                // TODO: Aside from short races, if this happens, we've got sockets without a doc.
                // Likely, we should reset all sockets.
                tracing::error!("Tried to store doc but project was None.");
                return;
            };
            let doc_box = project.doc_box.lock().await;
            let Some(doc_box) = doc_box.as_ref() else {
                // TODO: Aside from short races, if this happens, we've got sockets without a doc.
                // Likely, we should reset all sockets.
                tracing::error!("Tried to store doc but doc_box was None.");
                return;
            };
